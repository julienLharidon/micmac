...
3.  **Exécution du job** :
    ...
    **Note** : Le fichier `.rayignore` à la racine permet d'exclure les fichiers volumineux du repo MicMac original et de n'uploader que le nécessaire pour PyMicMac.
    ...
    # L'argument --runtime-env-json force Ray à lire le requirements.txt pour installer les dépendances sur le cluster.
    ray job submit --address http://localhost:8265 \
        --working-dir . \
        --runtime-env-json '{"pip": ["torch", "numpy", "ray[default]", "google-cloud-storage"]}' \
